...
for char in letters:
	atbash[char] = letters[len(letters)-1-letters.index(char)]
	atbash[char.upper()] = atbash[char].upper()

puzzle = 'gsrh rh zm vcznkov lu gsv zgyzhs xrksvi'
print(''.join([ atbash[x] if x in atbash else x for x in puzzle ]))
puzzle = 'GSRH rh zm vcznkov lu gsv ZGYZHS xrksvi'
print(''.join([ atbash[x] if x in atbash else x for x in puzzle ]))
