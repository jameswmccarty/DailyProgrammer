...
def roll(setting):
	times, sides = setting.split('d')
	rolls = [ random.randrange(int(sides)+1) for _ in range(int(times)) ]
	return (str(sum(rolls)), ': ' + ' '.join([str(_) for _ in rolls]))

roll_list = ['5d12','6d4','1d2','1d8','3d6','4d20','100d100']

for setting in roll_list:
	print(roll(setting))
