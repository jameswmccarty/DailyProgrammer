...
rate_limits = [(float("Inf"),0.4),(100000,0.25),(30000,0.1),(10000,0)]

def tax(income):
	rate_limits.sort(key=lambda x : x[0])
	owed = 0
	for idx in range(1,len(rate_limits)):
		if income >= rate_limits[idx-1][0]:
			owed += min(rate_limits[idx][0]-rate_limits[idx-1][0],(income - rate_limits[idx-1][0])) * rate_limits[idx][1]
	return int(owed)
