...
def max_increasing_seq(inpt,path=[]):
	
	if len(inpt) == 0:
		return path
	else:
		if len(path) == 0 or inpt[0] > path[-1]:
			a = max_increasing_seq(inpt[1:],path+[inpt[0]])
			b = max_increasing_seq(inpt[1:],path)
			if len(a) > len(b):
				return a
			return b
		return max_increasing_seq(inpt[1:],path)

print(max_increasing_seq([ int(x) for x in "0 8 4 12 2 10 6 14 1 9 5 13 3 11 7 15".split(' ') ]))
print(max_increasing_seq([ int(x) for x in "1 2 2 5 9 5 4 4 1 6".split(' ') ]))
print(max_increasing_seq([ int(x) for x in "4 9 4 9 9 8 2 9 0 1".split(' ') ]))
print(max_increasing_seq([ int(x) for x in "0 5 4 6 9 1 7 6 7 8".split(' ') ]))
print(max_increasing_seq([ int(x) for x in "1 2 20 13 6 15 16 0 7 9 4 0 4 6 7 8 10 18 14 10 17 15 19 0 4 2 12 6 10 5 12 2 1 7 12 12 10 8 9 2 20 19 20 17 5 19 0 11 5 20".split(' ') ]))
